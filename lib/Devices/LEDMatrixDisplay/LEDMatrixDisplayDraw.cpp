...
void LEDMatrixDisplay::drawFullscreen(const uint8_t* bitmap) {
    drawBitmap(bitmap, panelWidth, panelHeight, 0, 0);
}

void LEDMatrixDisplay::drawEye(const uint8_t* bitmap) {
    drawBitmap(bitmap, eyeWidth, eyeHeight, 7, 0);
}

void LEDMatrixDisplay::drawEye(const uint8_t* bitmapL, const uint8_t* bitmapR) {
    drawBitmap(bitmapL, bitmapR, eyeWidth, eyeHeight, 7, 0);
}

void LEDMatrixDisplay::drawEyePupil(const uint8_t* bitmap, int x, int y) {
    drawBitmap(bitmap, 6, 6, 21 + x, 5 + y);
}

void LEDMatrixDisplay::drawNose(const uint8_t* bitmap) {
    drawBitmap(bitmap, 8, 5, 56, 7);
}

void LEDMatrixDisplay::drawMouth(const uint8_t* bitmap) {
    drawBitmap(bitmap, mouthWidth, mouthHeight, 14, 18);
}

void LEDMatrixDisplay::drawColorTest() {
    uint8_t r, g, b;
    for (int i = 0; i < 64; i++) {
...
void LEDMatrixDisplay::drawBitmap(const uint8_t* bitmap, int imageWidth, int imageHeight, int offsetX, int offsetY) {
...
void LEDMatrixDisplay::drawBitmap(const uint8_t* bitmapL, const uint8_t* bitmapR, int imageWidth, int imageHeight, int offsetX, int offsetY) {
    for (int i = 0; i < imageHeight; i++) {
        int offsetYPlusI = offsetY + i;
        for (int j = 0, j2 = panelWidth - 1; j < imageWidth; j++, j2--) {
...
void LEDMatrixDisplay::drawBitmap(const uint8_t* bitmap, int imageWidth, int imageHeight, int offsetX, int offsetY, uint8_t r, uint8_t g, uint8_t b) {
    for (int i = 0; i < imageHeight; i++) {
        int offsetYPlusI = offsetY + i;
        for (int j = 0, j2 = panelWidth - 1; j < imageWidth; j++, j2--) {
            uint8_t pixel = pgm_read_byte(bitmap + i * imageWidth + j);
